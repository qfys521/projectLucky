...
public class LuckInteractor extends SimpleInteractors<LikeLucky> {
    @Filter("签到")
    public void lcs(XiaoMingUser user) {
        ...
        if(Long.valueOf(ft.format(dNow)).equals(plugin.datas.getLastDate(user.getCode()))){
            user.sendError("您今天已经过签到啦\n" +
                    "(当前您一共拥有:"+plugin.datas.getLuckyCount(user.getCode()) + "点Coin)");
        }else {
    ...
    public void setC(XiaoMingUser user, @FilterParameter("QQ") long qq, @FilterParameter("Count") long count) {
    ...
    @Filter("抽奖 {投入的数量}")
    public void dfp(XiaoMingUser user, @FilterParameter("投入的数量") long count) {
        if (count < 0) {
        ...
        }else if (plugin.datas.getLuckyCount(user.getCode()) < count) {
    ...
    @Filter("我的Coin")
    public void myCount(XiaoMingUser user) {
        user.sendMessage("您当前一共有" + plugin.datas.getLuckyCount(user.getCode()) + "枚Coin！");
    }

    @Filter("Coin查询 {QQ}")
    @Required("Lucky.admin.get")
    public void checkCoin(XiaoMingUser user, @FilterParameter("QQ") long qq) {
    ...
    public void isCOFCoinDisable(XiaoMingUser user, @FilterParameter("QQ") long qq) {
    ...
    public void isCOFCoinEnable(XiaoMingUser user, @FilterParameter("QQ") long qq) {
        plugin.datas.isCOFEnable(qq, true);
        user.sendMessage("神灵在世，祝福人间！");
    }

    @Filter("查询命运のLuckyCoin {QQ}")
    public void checkCOF(XiaoMingUser user, @FilterParameter("QQ") long qq) {
    ...
    public void zs(XiaoMingUser user, @FilterParameter("QQ") long qq, @FilterParameter("数量") int count) {
